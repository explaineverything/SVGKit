...
#import "SVGNamedNodeMap_Iterable.h" // Needed for the optional (non-SVG spec) "recursive toXML" method
...
- (id)initType:(DOMNodeType) nt name:(NSString*) n value:(NSString*) v
{
    ...
    if (self) {
		...
		self.childNodes = [[SVGNodeList alloc] init];
    }
    return self;
}
    ...
    if (self) {
        ...
        switch( nt )
		{
			...
			case DOMNodeType_ELEMENT_NODE:
			{
				...
				self.attributes = [[SVGNamedNodeMap alloc] init];
			}break;
		}
...
-(SVGNode*) replaceChild:(SVGNode*) newChild oldChild:(SVGNode*) oldChild
{
	if( newChild.nodeType == DOMNodeType_DOCUMENT_FRAGMENT_NODE )
	{
		...
		for( SVGNode* child in newChild.childNodes.internalArray )
		{
...
-(NSString *)textContent
{
	switch( self.nodeType )
	{
		...
		case DOMNodeType_DOCUMENT_FRAGMENT_NODE:
		{
			...
			for( SVGNode* subNode in self.childNodes.internalArray )
			{
...
//	NSAssert(namespaceShortnames != nil, @"Must supply an empty dictionary for me to fill with encountered namespaces, and the shortnames I invented for them!");
	...
	{
		SVGNode* attribute = [xmlnsNodemap objectForKey:xmlnsNodeName];
	...
	for( NSString* namespace in nodeMapsByNamespace )
	{
		...
		{
			SVGNode* attribute = [nodeMap objectForKey:nodeNameFromMap];
	...
	switch( self.nodeType )
	{
		...
		case DOMNodeType_ELEMENT_NODE:
		{
			for( SVGNode* child in self.childNodes )
			{
